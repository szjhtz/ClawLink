...
# Post-installation script for ClawLink on Linux
...
# Create symbolic link for ClawLink app binary
if [ -x /opt/ClawLink/clawlink ]; then
    ln -sf /opt/ClawLink/clawlink /usr/local/bin/clawlink 2>/dev/null || true
fi

# Create symbolic link for openclaw CLI
OPENCLAW_WRAPPER="/opt/ClawLink/resources/cli/openclaw"
...
if ! { [[ -L /proc/self/ns/user ]] && unshare --user true; }; then
    ...
    chmod 4755 '/opt/ClawLink/chrome-sandbox' || true
else
    chmod 0755 '/opt/ClawLink/chrome-sandbox' || true
fi
...
if apparmor_status --enabled > /dev/null 2>&1; then
    APPARMOR_PROFILE_SOURCE='/opt/ClawLink/resources/apparmor-profile'
    APPARMOR_PROFILE_TARGET='/etc/apparmor.d/clawlink'
...
echo "ClawLink has been installed successfully."
