...
// 7b. Fix ESM-only packages that are loaded via require() at runtime
const esmFixPackages = ['https-proxy-agent', 'http-proxy-agent', 'agent-base'];
for (const pkg of esmFixPackages) {
  const pkgDir = path.join(OUTPUT, 'node_modules', pkg);
  ...
  try {
    ...
    if (meta.type === 'module' && meta.exports && !meta.exports.require) {
      const esmEntry = meta.exports.import?.default || meta.exports.import || meta.exports['.']?.import?.default || meta.exports['.']?.import || meta.exports.default;
      if (esmEntry) {
        // Create CJS wrapper
        const wrapperFile = './_cjs_wrapper.cjs';
        fs.writeFileSync(path.join(pkgDir, '_cjs_wrapper.cjs'),
          `// Auto-generated CJS wrapper for ESM-only package\nconst mod = require("${esmEntry}");\nmodule.exports = mod;\nmodule.exports.default = mod;\n`);
        // Patch exports to add "require" entry (Node ignores "main" when "exports" exists)
        ...
        fs.writeFileSync(pkgJsonPath, JSON.stringify(meta, null, 2));
        echo`   🩹 Fixed ESM-only package: ${pkg}`;
      }
    }
  } catch {}
}
