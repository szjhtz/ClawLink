...
export const ChatMessage = memo(function ChatMessage({
  ...
  return (
    ...
    >
      ...
      <div
        className={cn(
          'flex h-7 w-7 shrink-0 items-center justify-center rounded-lg mt-1',
          isUser
            ? 'bg-primary/15'
            : 'bg-card border border-border',
        )}
      >
        {isUser ? <User className="h-3.5 w-3.5 text-primary" /> : <img src={logoPng} alt="" className="w-4 h-4" />}
      </div>

      {/* Content */}
      <div
        className={cn(
          'flex flex-col w-full min-w-0 max-w-[85%] space-y-1.5',
...
function MessageBubble({
  ...
  return (
    ...
    >
      {isUser ? (
        <p className="whitespace-pre-wrap break-words break-all text-[13px] leading-relaxed">{text}</p>
      ) : (
          ...
          <ReactMarkdown
            ...
            components={{
              code({ className, children, ...props }) {
                ...
                if (isInline) {
                  return (
                    <code className="bg-muted text-primary px-1.5 py-0.5 rounded text-xs font-mono break-words break-all" {...props}>
                      {children}
                    </code>
                  );
                }
                return (
                  <pre className="bg-muted rounded-lg p-3 whitespace-pre-wrap break-all overflow-hidden max-w-full">
                    <code className={cn('text-xs font-mono text-primary', className)} {...props}>
                      {children}
                    </code>
                  </pre>
                );
...
function ThinkingBlock({ content, defaultExpanded = false }: { content: string; defaultExpanded?: boolean }) {
  ...
  return (
    <div className="w-full rounded-[10px] border border-border bg-card text-[13px]">
      <button
      ...
      {expanded && (
        <div className="px-3 pb-3 text-muted-foreground">
          <div className="prose prose-sm dark:prose-invert max-w-none opacity-75 text-[12px] leading-relaxed">
...
function ImagePreviewCard({
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 bg-black/0 group-hover/img:bg-foreground/20 transition-colors flex items-center justify-center">
        <ZoomIn className="h-6 w-6 text-white opacity-0 group-hover/img:opacity-100 transition-opacity drop-shadow" />
      </div>
    </div>
  );
        ...
        <div className="flex items-center gap-2">
          {filePath && (
            <Button
              variant="ghost"
              size="icon"
              className="h-8 w-8 bg-foreground/10 hover:bg-foreground/20 text-white"
...
function ToolCard({ name, input }: { name: string; input: unknown }) {
  ...
  return (
    <div className="rounded-lg border border-border/50 bg-muted/20 text-sm overflow-hidden max-w-full">
      <button
      ...
      {expanded && input != null && (
        <pre className="px-3 pb-2 text-xs text-muted-foreground whitespace-pre-wrap break-all overflow-hidden max-w-full">
