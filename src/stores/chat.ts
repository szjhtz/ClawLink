...
interface ChatState {
  ...
  handleThinkingStream: (text: string, runId: string, sessionKey?: string) => void;
  toggleThinking: () => void;
...
const IMAGE_CACHE_KEY = 'clawlink:image-cache';
...
export const useChatStore = create<ChatState>((set, get) => ({
  ...
  loadSessions: async () => {
    // Check Gateway status before making RPC call
    const gatewayStatus = useGatewayStore.getState().status;
    if (gatewayStatus.state !== 'running') {
      console.warn('[chat.loadSessions] Gateway not running, state:', gatewayStatus.state);
      return; // Silent fail, sessions will load when Gateway becomes ready
    }

    try {
      // Add timeout to prevent hanging forever
      const timeoutMs = 10000;
      const data = await useGatewayStore.getState().rpc<Record<string, unknown>>('sessions.list', {}, timeoutMs);
      if (data) {
        ...
        // load labels in background (non-blocking; list shows first, labels update progressively)
        const sessionsToLabel = sessionsWithCurrent.filter(
          (s) => !s.key.endsWith(':main') && !get().sessionLabels[s.key]
        );
        if (sessionsToLabel.length > 0) {
          // load sequentially to avoid overwhelming Gateway
          void (async () => {
  ...
  loadHistory: async (quiet = false) => {
    ...
    const gatewayStatus = useGatewayStore.getState().status;
    if (gatewayStatus.state !== 'running') {
      console.warn('[chat.loadHistory] Gateway not running, state:', gatewayStatus.state);
      if (!quiet) set({ loading: false, error: 'Gateway not running. Please start Gateway and try again.' });
      return;
    }
    ...
    try {
      ...
      if (!data) {
        // don't clear messages while sending; keep optimistic user message
        if (!get().sending) {
      ...
      if (data) {
        ...
        // When sending, only update messages if the server has new content
        // (e.g. assistant reply). Otherwise keep local state to avoid
        // the optimistic user message flashing (different IDs cause React remount).
        let finalMessages = enrichedMessages;
        if (get().sending) {
          const currentMsgs = get().messages;
          const hasNewAssistant = enrichedMessages.some(
            (m) => m.role === 'assistant' && !currentMsgs.some(
              (cm) => cm.role === 'assistant' && cm.timestamp === m.timestamp,
            ),
          );
          if (!hasNewAssistant && currentMsgs.length > 0) {
            // no new assistant messages from server → keep local state to avoid flicker
            set({ loading: false });
            return;
          }
          // new assistant messages found → replace with server data (full conversation)
        ...
        // displayName (e.g. the configured agent name "ClawLink") instead.
        ...
        // don't clear messages while sending
        if (!get().sending) {
          set({ messages: [], loading: false });
        } else {
          set({ loading: false });
      ...
      console.error('[chat.loadHistory] Failed to load history:', err);
      const errorMsg = err instanceof Error ? err.message : String(err);
      const isSending = get().sending;
      // don't clear messages while sending; only set loading and error
      const msgUpdate = isSending ? {} : { messages: [] as RawMessage[] };
      // Provide more helpful error messages based on the error
      if (errorMsg.includes('timeout') || errorMsg.includes('Timeout')) {
        set({ ...msgUpdate, loading: false, error: 'Loading history timed out. Please check if Gateway is running.' });
      } else if (errorMsg.includes('Gateway not connected') || errorMsg.includes('Gateway not connected')) {
        set({ ...msgUpdate, loading: false, error: 'Gateway not connected. Please start Gateway and try again.' });
      } else {
        set({ ...msgUpdate, loading: false, error: `Failed to load history: ${errorMsg}` });
      }
    }
  },
  ...
  handleThinkingStream: (text: string, _runId: string, sessionKey?: string) => {
    const { currentSessionKey, sending } = get();
    // only process thinking stream for the current session
    if (sessionKey && sessionKey !== currentSessionKey) return;
    if (!sending) return;

    // construct or update streamingMessage, injecting thinking content as a thinking block
    set((s) => {
      const current = s.streamingMessage as RawMessage | null;
      const thinkingBlock = { type: 'thinking' as const, thinking: text };

      if (current && typeof current === 'object') {
        // existing streamingMessage — update the thinking block in its content
        const existingContent = Array.isArray((current as RawMessage).content)
          ? [...((current as RawMessage).content as ContentBlock[])]
          : [];
        // replace or append thinking block
        const thinkingIdx = existingContent.findIndex((b: ContentBlock) => b.type === 'thinking');
        if (thinkingIdx >= 0) {
          existingContent[thinkingIdx] = thinkingBlock;
        } else {
          existingContent.unshift(thinkingBlock);
        }
        return {
          streamingMessage: { ...current, content: existingContent } as unknown,
        };
      }

      // no streamingMessage yet — create a new one
      return {
        streamingMessage: {
          role: 'assistant',
