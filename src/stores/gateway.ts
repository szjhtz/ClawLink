...
function handleGatewayNotification(notification: { method?: string; params?: Record<string, unknown> } | undefined): void {
  ...
  // handle thinking stream events (stream: "thinking")
  const streamType = p.stream ?? data.stream;
  if (streamType === 'thinking') {
    const thinkingText = data.text ?? (p.data as Record<string, unknown>)?.text;
    if (typeof thinkingText === 'string') {
      import('./chat')
        .then(({ useChatStore }) => {
          useChatStore.getState().handleThinkingStream(
            thinkingText,
            String(p.runId ?? data.runId ?? ''),
            p.sessionKey != null ? String(p.sessionKey) : undefined,
          );
        })
        .catch(() => {});
    }
...
function handleGatewayChatMessage(data: unknown): void {
  ...
  // Check if this is a ClawLink background session reply
  const eventSessionKey = chatData.sessionKey != null ? String(chatData.sessionKey) : (payload.sessionKey != null ? String(payload.sessionKey) : null);
  const eventState = String(payload.state || chatData.state || '');
  if (eventSessionKey && (eventState === 'final' || (!eventState && payload.role === 'assistant'))) {
    import('./clawlink').then(({ useClawLinkStore }) => {
      const clawStore = useClawLinkStore.getState();
      if (clawStore.autoReplying) {
        const isClawLinkSession = clawStore.clawLinkSessions.some(s => s.openclawSessionKey === eventSessionKey);
        if (isClawLinkSession) {
          // Extract reply content from the event
