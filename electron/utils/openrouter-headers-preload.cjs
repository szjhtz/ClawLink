 ...
 * "openrouter.ai" carries the ClawLink app-attribution headers.
 *
...
(function () {
  ...
  if (globalThis.__clawLinkFetchPatched) return;
  globalThis.__clawLinkFetchPatched = true;

  globalThis.fetch = function clawLinkFetch(input, init) {
    var url =
    ...
    if (url.indexOf('openrouter.ai') !== -1) {
      ...
      delete flat['x-title'];
      delete flat['X-Title'];
      flat['HTTP-Referer'] = 'https://github.com/CN-Syndra/ClawLink';
      flat['X-Title'] = 'ClawLink';
      init.headers = flat;
    }
