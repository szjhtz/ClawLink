...
/**
 * Write the ClawLink gateway token into ~/.openclaw/openclaw.json.
 */
export async function syncGatewayTokenToConfig(token: string): Promise<void> {
  ...
  // Packaged ClawLink loads the renderer from file://, so the gateway must allow
 ...
 * OpenClaw/ClawLink versions or external tools.
 *
...
export async function sanitizeOpenClawConfig(): Promise<void> {
  ...
  // When ClawLink-managed moonshot provider exists, prefer centralized key
