...
import { getClawLinkProviderStore } from '../services/providers/store-instance';
import {
...
export async function storeApiKey(providerId: string, apiKey: string): Promise<boolean> {
  try {
    ...
    const s = await getClawLinkProviderStore();
 ...
 * Also synchronizes ClawLink local provider list with OpenClaw's actual config.
 */
...
> {
  ...
  for (const provider of providers) {
    ...
    // we should remove it from ClawLink UI to stay consistent.
    ...
    if (!isBuiltin && !activeOpenClawProviders.has(provider.type) && !activeOpenClawProviders.has(provider.id) && !activeOpenClawProviders.has(openClawKey)) {
      console.log(`[Sync] Provider ${provider.id} (${provider.type}) missing from OpenClaw, dropping from ClawLink UI`);
