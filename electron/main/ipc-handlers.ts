...
function registerCronHandlers(gatewayManager: GatewayManager): void {
  ...
  // UI-created tasks have no delivery target — results go to the ClawLink chat page.
  ...
  ipcMain.handle('cron:create', async (_, input: {
    ...
    try {
      const gatewayInput = {
        ...
        wakeMode: 'next-heartbeat',
        sessionTarget: 'isolated',
        // UI-created jobs deliver results via ClawLink WebSocket chat events,
...
function registerOpenClawHandlers(gatewayManager: GatewayManager): void {
  ...
  // Save ClawLink current user info to local file (~/.openclaw/clawlink-current-user.json)
  ipcMain.handle('clawlink:saveCurrentUser', async (_, params: {
    userId: string;
    username: string;
    displayName: string;
    agentId: string;
    ...
    try {
      ...
      const filePath = join(dir, 'clawlink-current-user.json');
      const data = JSON.stringify(params, null, 2);
      writeFileSync(filePath, data, 'utf-8');
      return { success: true };
    } catch (error) {
      console.error('[IPC] Failed to save ClawLink current user:', error);
      return { success: false, error: String(error) };
    }
  });

  // Save ClawLink last sent message to local file (~/.openclaw/clawlink-last-sent.json)
  // This is used by the auto-reply feature to provide context when starting a new conversation
  ipcMain.handle('clawlink:saveLastSentMessage', async (_, params: {
    content: string;
    toAgentId: string;
  }) => {
    try {
      ...
      const filePath = join(dir, 'clawlink-last-sent.json');
      const data = JSON.stringify(params, null, 2);
      writeFileSync(filePath, data, 'utf-8');
      console.log('[IPC] Saved ClawLink last sent message to:', filePath);
      return { success: true };
    } catch (error) {
      console.error('[IPC] Failed to save ClawLink last sent message:', error);
      return { success: false, error: String(error) };
    }
  });

  // Get ClawLink last sent message from local file (~/.openclaw/clawlink-last-sent.json)
  ipcMain.handle('clawlink:getLastSentMessage', async () => {
    try {
      const { readFileSync, existsSync } = await import('node:fs');
      const filePath = join(homedir(), '.openclaw', 'clawlink-last-sent.json');
      ...
      console.error('[IPC] Failed to get ClawLink last sent message:', error);
      return { success: false, error: String(error), data: null };
    }
  });

  // Download a remote file to local staging directory for AI processing
  ipcMain.handle('clawlink:stageFromUrl', async (_, params: { url: string; fileName: string; mimeType: string }) => {
    try {
      const { writeFileSync, mkdirSync, existsSync } = await import('node:fs');
      const { join } = await import('node:path');
      const { homedir } = await import('node:os');

      const stagingDir = join(homedir(), '.openclaw', 'media', 'outbound');
      ...
      const stagedPath = join(stagingDir, `${Date.now()}-${params.fileName}`);
      writeFileSync(stagedPath, buffer);

      return { success: true, stagedPath };
    } catch (error) {
      return { success: false, error: String(error) };
    }
  });

  // Download a ClawLink file to ~/Downloads/ClawLink/ and reveal in Finder/Explorer
  ipcMain.handle('clawlink:downloadAndReveal', async (_, params: { url: string; fileName: string }) => {
    try {
      ...
      const downloadDir = join(homedir(), 'Downloads', 'ClawLink');
      if (!existsSync(downloadDir)) {
        mkdirSync(downloadDir, { recursive: true });
      }

      // If file already exists, skip download and return existing path
      const finalName = params.fileName;
      const finalPath = join(downloadDir, finalName);
      if (existsSync(finalPath)) {
        return { success: true, path: finalPath, skipped: true };
      }
  ...
  // Upload a local file to the ClawLink server
  ipcMain.handle('clawlink:uploadFile', async (_, params: { filePath: string; fileName: string; mimeType: string; serverUrl: string; token: string }) => {
    try {
      ...
      const resp = await fetch(`${params.serverUrl}/api/files`, {
        method: 'POST',
  ...
  // Force deploy all resources (skills + prompts) - overwrites existing
  ipcMain.handle('clawlink:deployResources', async () => {
    try {
      const { forceDeployResources } = await import('../utils/skill-config');
      const result = await forceDeployResources();
      return { success: true, ...result };
    } catch (error) {
      console.error('[IPC] clawlink:deployResources failed:', error);
      return { success: false, error: String(error) };
    }
  });

  // Factory reset — clear all settings, storage, and redeploy resources
  ipcMain.handle('clawlink:factoryReset', async () => {
    try {
      const { resetSettings } = await import('../utils/store');
      await resetSettings();

      const { forceDeployResources } = await import('../utils/skill-config');
      await forceDeployResources();

      // Clear clawlink user config file
      const { unlinkSync, existsSync } = await import('node:fs');
      const userConfigPath = join(homedir(), '.openclaw', 'clawlink-current-user.json');
      if (existsSync(userConfigPath)) unlinkSync(userConfigPath);

      return { success: true };
    } catch (error) {
      console.error('[IPC] clawlink:factoryReset failed:', error);
      return { success: false, error: String(error) };
    }
  });

  // Get ClawLink prompts config - loads from ~/.openclaw/clawlink-prompts/
  // Re-reads files on every call to support hot-reload
  ipcMain.handle('clawlink:getPrompts', async () => {
    try {
      const { readFileSync, existsSync, mkdirSync, cpSync, readdirSync } = await import('node:fs');

      const promptsDir = join(homedir(), '.openclaw', 'clawlink-prompts');

      const requiredFiles = ['system-prompt.txt', 'followup-ready.txt', 'followup-normal.txt', 'delimiter-start.txt', 'delimiter-end.txt', 'topic-comment.txt'];
      const needsSetup = !existsSync(promptsDir) || !existsSync(join(promptsDir, 'system-prompt.txt'));
      ...
      if (needsSetup) {
        ...
        const bundledPaths = app.isPackaged
          ? [
              join(process.resourcesPath || '', 'resources', 'clawlink-prompts'),
              join(process.resourcesPath || '', 'app.asar.unpacked', 'resources', 'clawlink-prompts'),
            ]
          : [
              join(app.getAppPath(), 'resources', 'clawlink-prompts'),
              join(process.cwd(), 'resources', 'clawlink-prompts'),
            ];

        for (const bundledPath of bundledPaths) {
          if (existsSync(bundledPath)) {
            try {
              const files = readdirSync(bundledPath);
      ...
      const files = ['system-prompt.txt', 'followup-ready.txt', 'followup-normal.txt', 'delimiter-start.txt', 'delimiter-end.txt', 'topic-comment.txt'];

      for (const file of files) {
        const filePath = join(promptsDir, file);
        if (existsSync(filePath)) {
          prompts[file.replace('.txt', '')] = readFileSync(filePath, 'utf-8').trim();
        }
      }

      if (Object.keys(prompts).length === 0) {
        return { success: false, error: 'No prompts found', prompts: null };
      }
      return { success: true, prompts, source: promptsDir };
    } catch (error) {
      console.error('[IPC] Failed to load ClawLink prompts:', error);
      return { success: false, error: String(error), prompts: null };
    }
  });
  ...
  ipcMain.handle(
    ...
    async (
      ...
      try {
        ...
        console.log(`[clawlink-validate] validating provider type: ${providerType}`);
...
function registerShellHandlers(): void {
  // Open external URL (restrict to http/https only)
