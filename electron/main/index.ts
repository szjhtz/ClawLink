...
import { ensureClawLinkContext, repairClawLinkOnlyBootstrapFiles } from '../utils/openclaw-workspace';
...
// On Wayland this maps the running window to clawlink.desktop (→ icon + app grouping);
// on X11 it supplements the StartupWMClass matching.
// Must be called before app.whenReady() / before any window is created.
if (process.platform === 'linux') {
  app.setDesktopName('clawlink.desktop');
}
...
async function initialize(): Promise<void> {
  ...
  logger.init();
  logger.info('=== ClawLink Application Starting ===');
  logger.debug(
  ...
  // Repair any bootstrap files that only contain ClawLink markers (no OpenClaw
  // template content). This fixes a race condition where ensureClawLinkContext()
  // previously created the file before the gateway could seed the full template.
  void repairClawLinkOnlyBootstrapFiles().catch((error) => {
    logger.warn('Failed to repair bootstrap files:', error);
  });

  // Deploy built-in skills and prompts on every startup
  // This ensures resources are always up-to-date after an app update
  void (async () => {
    try {
      ...
      logger.warn('Failed to deploy resources on startup:', error);
    }
  })();
  ...
  gatewayManager.on('status', (status: { state: string }) => {
    ...
    if (status.state === 'running') {
      void ensureClawLinkContext().catch((error) => {
        logger.warn('Failed to re-merge ClawLink context after gateway reconnect:', error);
      });
    }
  });
  ...
  // Merge ClawLink context snippets into the workspace bootstrap files.
  // The gateway seeds workspace files asynchronously after its HTTP server
  // is ready, so ensureClawLinkContext will retry until the target files appear.
  void ensureClawLinkContext().catch((error) => {
    logger.warn('Failed to merge ClawLink context into workspace:', error);
  });
