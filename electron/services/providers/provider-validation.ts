...
function logValidationStatus(provider: string, status: number): void {
  console.log(`[clawlink-validate] ${provider} HTTP ${status}`);
}
...
function logValidationRequest(
  ...
  console.log(
    `[clawlink-validate] ${provider} request ${method} ${sanitizeValidationUrl(url)} headers=${JSON.stringify(sanitizeHeaders(headers))}`,
  );
}
...
async function validateOpenAiCompatibleKey(
  ...
  if (modelsResult.error?.includes('API error: 404')) {
    console.log(
      `[clawlink-validate] ${providerType} /models returned 404, falling back to /chat/completions probe`,
    );
